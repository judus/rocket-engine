export default class PlayerActions {
    ...
    setupEventListeners() {
        ...
        this.eventBus.on('scopedMouseDownPrimary', (event) => {
            const {scopedMouse, element} = event;
            this.attack(event);

            // Example of scoping actions to different canvases
            // if(element === canvas1) {
            //     // Do something this
            // } else if(element === canvas2) {
            //     // Do something that
            // }
        });

        this.eventBus.on('scopedMouseDownSecondary', (event) => {
            const {scopedMouse, element} = event;
            this.contextClick(event);
